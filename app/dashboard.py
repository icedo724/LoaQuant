...
def get_prophet_forecast(series_data, periods=7):
    """
    Prophet 라이브러리를 사용하여 단기 시세 예측을 수행하는 함수
    주간 계절성(weekly_seasonality)을 반영하여 n일간의 미래 데이터를 추정
    """
...
def add_smart_event_logs(fig, event_logs, min_date, max_date, y_pos=1.05):
    """
    그래프 내 특정 날짜에 이벤트 라인 및 텍스트 주석을 그리는 함수
    동일한 날짜에 이벤트가 여러 개일 경우 병합하여 출력
    """
...
def load_event_logs():
    """패치/업데이트 등 이벤트 로그를 텍스트 파일에서 읽어오는 함수"""
    events = {}
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(current_dir)
...
def preprocess_for_chart(df, selected_items):
    """차트 렌더링을 위해 시계열 인덱스로 데이터를 변환 (행/열 전치)"""
    # Numpy Array 에러 방지를 위해 len() == 0 으로 체크
...
def apply_gold_conversion(df, gold_dict, latest_gold):
    """현재 시세(골드)를 날짜별 현금(원) 가치로 스케일링하여 반환"""
    df_conv = df.copy()
...
def get_loa_daily_avg_df(df):
    """로스트아크 서버 기준일(오전 6시)에 맞추어 일평균 가격 산출"""
...
def analyze_market_status(df, column_name, is_cash=False):
    """
    아이템의 현재 RSI 지표와 볼린저 밴드를 계산하여
    매수/매도/관망 등의 시장 시그널과 색상 상태를 반환
    """
...
def draw_stock_chart(df, title_text="", is_cash=False):
    ...
    # 정기 점검 시간 음영 처리 (수요일 06:00 ~ 10:00)
...
def draw_day_of_week_chart(df, title_text="", is_cash=False):
    """주간 초기화일(수요일) 기준 타 요일의 변동률을 막대 그래프로 렌더링"""
...
def draw_summary_table(df, is_cash=False):
    """일별 가격 및 전일 대비 등락폭을 데이터프레임 형태 표로 출력"""
    daily_df = get_loa_daily_avg_df(df)
    if daily_df.empty: return
...
def draw_volume_chart(df_vol, selected_items, title_text=""):
    """API를 통해 수집된 일일 거래량 추이를 막대 그래프로 렌더링"""
    # Numpy Array 에러 방지를 위해 len() == 0 으로 체크
...
# 7개의 메인 탭 구조 설정
tab_gold, tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["골드 시세", "강화 재료", "생활 재료", "배틀 아이템", "각인서", "보석", "거래대금"])
...
with tab1:
    ...
    if df_materials is not None:
        ...
        if not chart_data.empty:
            ...
            # 하위 재료 5개와 상위 재료 1개의 효율 교환 비율 분석 표시
